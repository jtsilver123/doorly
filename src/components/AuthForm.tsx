...
export default function AuthForm({
  ...
  /*
   * Google leads; email folds away.
   *
   * One tap that brings its own name and verified address beats three fields
   * and a password nobody wanted to invent — so the page offers exactly one
   * obvious thing to do. The email form is still fully there, one click away,
   * and it unfolds itself when there's an error to show (a failed password
   * attempt must never hide the field it happened in).
   */
  const [emailOpen, setEmailOpen] = useState(Boolean(initialError));
      ...
      <section className="auth-panel">
        <form className="auth-form" action={formAction}>
          ...
          */}
          <button
            type="button"
            className="btn auth-google auth-google-lead"
            disabled={oauthBusy}
            onClick={async () => {
              setOauthBusy(true);
          ...
          {!emailOpen ? (
            <button
              type="button"
              className="btn auth-email-toggle"
